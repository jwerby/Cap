...
export const WebRecorderDialog = () => {
	...
	const handleOpenChange = (next: boolean) => {
		if (next && supportCheckCompleted && !isBrowserSupported) {
			toast.error(
				"This browser isn't compatible with Port & Starboard Watch recorder. We recommend Google Chrome or other Chromium-based browsers.",
			);
			return;
		}
									...
									<div className="rounded-md border border-blue-6 bg-blue-3/60 px-3 py-2">
										...
										<div className="mt-2 flex flex-col gap-2">
											{recoveredDownloads.map((download) => (
												<div
													key={download.id}
													className="flex items-center justify-between gap-3 rounded-md bg-white/70 px-2.5 py-2 text-xs text-gray-12 dark:bg-gray-2"
												>
