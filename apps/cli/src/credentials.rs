...
const DEFAULT_SERVER: &str = "https://watch.portstbd.com";
...
pub fn resolve() -> Result<Credentials, String> {
    ...
    Err(
        "Not signed in. Sign in to the Port and Starboard Watch desktop app (the CLI reuses its \
         login), or set CAP_API_KEY to an API key from Settings."
            .to_string(),
    )
}
        ...
        OutputFormat::Text => {
            if status.authenticated {
                let source = match status.source {
                    ...
                    CredentialSource::Desktop => "Watch desktop login",
