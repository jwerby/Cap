...
describe("InstantRecordingUploader", () => {
	...
	it("normalizes multipart initiate content type before creating the upload", async () => {
		...
		const { initiateMultipartUpload } = await import(
			"@cap/recorder-core/instant-mp4-uploader"
		);
	...
	it("marks the uploader as fatal after the final retry fails", async () => {
		...
		vi.stubGlobal("fetch", fetchMock);
		// One outcome per attempt; part uploads retry up to 8 times so a brief
		// network blip cannot kill a live recording.
		MockXMLHttpRequest.setOutcomes(
			Array.from({ length: 8 }, () => ({ type: "network-error" as const })),
		);
	...
	it("treats interrupted multipart completion as uncertain instead of fatal cleanup", async () => {
		...
		const finalizeExpectation = expect(finalizePromise).rejects.toBeInstanceOf(
			MultipartCompletionUncertainError,
		);
		...
		// The completion call is the one request whose loss can strand a
		// finished upload, so transient failures are retried before giving up.
